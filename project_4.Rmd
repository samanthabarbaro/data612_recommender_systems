...
The predicted lists are similar for these users, excluding shows or movies they've already seen. The RMSE is lower than UBCF, and precision/recall are higher. The same is true for CBF. SVD's RMSE comes close, so while the model is bad at picking true positives (choosing items the user has watched) it's good at making accurate predictions.
...
The RMSE came down a bit, and it's a little closer to popular and SVD. Precision and recall both went down as well.
